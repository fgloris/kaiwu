...
###########################################################################
"""
Feature preprocessor and reward design for Gorge Chase PPO V6.
在你现有 V5 上增强：
1. 宝箱 / buff 记忆缓存；
2. 探索与反绕圈；
3. 10 步位移特征与 anti-stall 惩罚；
4. 访问热度 local patch；
5. 奖励更明确地区分“推进拿分”和“高压保命”。
"""
...
class Preprocessor:
    ...
    def _entity_key(self, ent, default_prefix, hero_x, hero_z):
        cid = ent.get("config_id", None)
        if cid is not None:
            return f"{default_prefix}_{int(cid)}"
        x, z = self._entity_position(ent, hero_x, hero_z)
        return f"{default_prefix}_{int(round(x))}_{int(round(z))}"
    ...
    def _refresh_memory(self, visible_treasures, visible_buffs, env_info, hero_x, hero_z):
        ...
        # 按 remaining treasure ids 清掉已被吃掉的缓存
        if remaining_ids:
            remove_keys = []
            for key in self.treasure_memory.keys():
                if key.startswith("treasure_"):
                    suffix = key.split("treasure_", 1)[1]
    ...
    def _memory_entities(self, visible_entities, memory_dict, prefix, hero_x, hero_z):
        ...
        for key, (x, z) in memory_dict.items():
            if key in visible_keys:
                continue
            merged.append({"pos": {"x": x, "z": z}, "status": 1, "memory": 1})
        return merged
    ...
    def _move_safety(self, map_info, hero_x, hero_z, monster_positions):
        ...
        for dx, dz in ACTION_DIRS:
            ...
            score = 0.62 * np.clip(min_dist / 35.0, 0.0, 1.0) + 0.23 * (open_len / 6.0) + 0.15 * novelty
    ...
    def _flash_safety(self, map_info, hero_x, hero_z, monster_positions):
        ...
        for dx, dz in ACTION_DIRS:
            ...
            open_len = self._open_length(map_info, int(np.sign(fx)), int(np.sign(fz)), max_len=6)
            distance_bonus = min(abs(fx) + abs(fz), 12.0) / 12.0
            novelty = 1.0 - min(self.visited_count.get((nx, nz), 0) / 3.0, 1.0)
            score = 0.52 * np.clip(min_dist / 45.0, 0.0, 1.0) + 0.18 * (open_len / 6.0) + 0.18 * distance_bonus + 0.12 * novelty
            scores.append(float(np.clip(score, 0.0, 1.0)))
    ...
    def feature_process(self, env_obs, last_action):
        ...
        if len(self.recent_exact_positions) > Config.STALL_WINDOW:
            ...
            if _l2(hero_x, hero_z, old_x, old_z) < Config.STALL_DIST_THRESHOLD:
                anti_stall_penalty = -0.06
        ...
        visible_treasures_count = len(visible_treasures)
        memory_treasure_count = len(self.treasure_memory)
        memory_buff_count = len(self.buff_memory)
        global_feat = np.array([
            ...
            min(memory_treasure_count / 10.0, 1.0),
        ], dtype=np.float32)
        ...
        survive_reward = 0.016 if high_pressure > 0.5 else 0.010
        ...
        treasure_score_reward = 2.25 * min(treasure_score_gain / 100.0, 2.0) if treasure_score_gain > 0 else 0.0
        ...
        if best_treasure_dist is not None and self.last_best_treasure_dist is not None and self.last_min_monster_dist_norm is not None:
            ...
            if approach > 0 and monster_closer < 0.04:
                ...
                treasure_approach_reward = 0.08 * phase_scale * best_treasure_priority * float(np.clip(approach / 5.0, 0.0, 1.0))
            if approach > 0 and monster_closer > 0.05 and cur_min_monster_dist_norm < 0.24:
                risky_treasure_penalty = -0.05 * min(monster_closer / 0.10, 1.0)

        priority_tracking_reward = 0.0
        if self.last_best_treasure_priority is not None and best_treasure_priority > 0 and high_pressure < 0.5:
        ...
        if pre_speedup_urgency > 0.55 and self.last_min_monster_dist_norm is not None:
            dist_delta = cur_min_monster_dist_norm - self.last_min_monster_dist_norm
            if dist_delta > 0:
                speedup_buffer_reward = 0.07 * pre_speedup_urgency * dist_delta

        first_seen_treasure_reward = 0.0
        if newly_seen_treasure > 0 and high_pressure < 0.5:
            first_seen_treasure_reward = 0.045 * min(newly_seen_treasure, 2)

        exploration_reward = 0.0
        if cur_min_monster_dist_norm > (0.18 if high_pressure < 0.5 else 0.24):
            if prev_visit == 0:
                exploration_reward = 0.028 if high_pressure < 0.5 else 0.012
            elif prev_visit <= 2 and high_pressure < 0.5:
                exploration_reward = 0.008

        corridor_reward = 0.025 * corridor_score * (0.4 + 0.6 * high_pressure)
        deadend_penalty = 0.0
        if corridor_score < 0.28 and cur_min_monster_dist_norm < 0.22:
            deadend_penalty = -0.09 * (0.28 - corridor_score) / 0.28

        trap_penalty = -0.11 * trap_pressure * (0.5 + 0.5 * high_pressure)

        danger_penalty = 0.0
        danger_threshold = 0.22 if high_pressure < 0.5 else 0.28
        if cur_min_monster_dist_norm < danger_threshold:
            danger_penalty = -0.13 * (danger_threshold - cur_min_monster_dist_norm) / danger_threshold
        ...
        if repeat_ratio > 0.55:
            repeat_penalty = -0.035 * (repeat_ratio - 0.55) / 0.45
        ...
        if last_action >= 8 and self.last_min_monster_dist_norm is not None:
            ...
            if dist_gain > 0.04 or corridor_gain > 0.08 or treasure_score_gain > 0 or buff_delta > 0:
                flash_reward = 0.22 + 0.40 * max(dist_gain, 0.0) + 0.12 * max(corridor_gain, 0.0)
                if treasure_score_gain > 0 or buff_delta > 0:
                    flash_reward += 0.08
            else:
                flash_reward = -0.07
