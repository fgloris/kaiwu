...
Structured neural network model for Gorge Chase PPO V5.
峡谷追猎 PPO V5 结构化神经网络模型：MLP + CNN。
"""
...
class AttnPool(nn.Module):
    """Tiny attention pooling over a small set of entities."""
...
class Model(nn.Module):
    def __init__(self, device=None):
        super().__init__()
        self.model_name = "gorge_chase_v5_mlp_cnn"
        self.device = device

        action_num = Config.ACTION_NUM
        value_num = Config.VALUE_NUM

        # Structured scalar encoders
        self.hero_encoder = MLPBlock(Config.HERO_DIM, 32, 32)
        self.monster_encoder = MLPBlock(Config.MONSTER_DIM, 24, 24)
        self.treasure_encoder = MLPBlock(Config.TREASURE_DIM, 20, 20)
        self.buff_encoder = MLPBlock(Config.BUFF_DIM, 16, 16)
        self.move_encoder = MLPBlock(Config.MOVE_SAFETY_DIM, 24, 24)
        self.flash_encoder = MLPBlock(Config.FLASH_SAFETY_DIM, 24, 24)
        self.global_encoder = MLPBlock(Config.GLOBAL_DIM, 32, 32)

        # Query-based pooling for paired entities
        self.monster_pool = AttnPool(entity_dim=24, query_dim=64, hidden_dim=24)
        self.treasure_pool = AttnPool(entity_dim=20, query_dim=64, hidden_dim=20)
        self.buff_pool = AttnPool(entity_dim=16, query_dim=64, hidden_dim=16)

        # Local map CNN
        self.map_encoder = MapCNN(in_ch=Config.MAP_CHANNEL)

        fused_dim = 32 + 24 + 20 + 16 + 24 + 24 + 32 + 64
